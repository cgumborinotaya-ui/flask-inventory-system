...
def index():
    ...
    return render_template('index.html', assets=assets, stats=stats, type_counts=type_counts, top_provinces=top_provinces)
...
def edit_asset(id):
    ...
    if request.method == 'POST':
        ...
        antivirus_license_date_str = request.form.get('antivirus_license_date')
        asset.office_name = request.form.get('office_name')
        office_license_date_str = request.form.get('office_license_date')
        asset.os_name = request.form.get('os_name')
        u = current_user()
        ...
        if u and u.role != 'IT' and u.province and u.province != 'Head Office':
            asset.province = u.province
        else:
            asset.province = request.form.get('province')
        if u:
            if u.role == 'AdminDistrict' and u.district:
                if posted_district and posted_district != u.district:
                    attempted_relocate = True
                asset.district = u.district
            else:
                asset.district = request.form.get('district')
        else:
            asset.district = request.form.get('district')
        asset.inspected_by_ict = True if request.form.get('inspected_by_ict') == 'on' else False
        inspection_date_str = request.form.get('inspection_date')

        try:
            asset.purchase_date = datetime.strptime(purchase_date_str, '%Y-%m-%d').date() if purchase_date_str else None
            asset.antivirus_license_date = datetime.strptime(antivirus_license_date_str, '%Y-%m-%d').date() if antivirus_license_date_str else None
            asset.office_license_date = datetime.strptime(office_license_date_str, '%Y-%m-%d').date() if office_license_date_str else None
            if is_it():
                asset.inspection_date = datetime.strptime(inspection_date_str, '%Y-%m-%d').date() if inspection_date_str else None
            else:
                asset.inspected_by_ict = asset.inspected_by_ict  # ignore changes by non-IT
                asset.inspection_date = asset.inspection_date
...
def delete_asset(id):
    ...
    try:
        ...
        db.session.commit()
        log_action('delete_asset', 'Asset', id, asset.name)
        flash('Asset deleted successfully!', 'success')
    except Exception as e:
        log_action('delete_asset_error', 'Asset', id, str(e))
...
def get_report_assets(report_type):
    ...
    if report_type == 'computers_health':
...
def delete_user(id):
    ...
    if count_assets > 0:
        log_action('delete_user_rejected', 'User', user.id, f'assets:{count_assets}')
        flash('Cannot delete user with captured assets', 'danger')
        return redirect(url_for('users'))
